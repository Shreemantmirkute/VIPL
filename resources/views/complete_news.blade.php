    ...
    <div id="heroCarousel" class="container carousel carousel-fade" data-ride="carousel">
      ...
      <div class="carousel-item active">
        <div class="carousel-container">
          <h2 class="animated fadeInDown">News</span></h2>
        ...
        <div class="row">
          ...
          <!-- <div class="col-sm-4">
              <?php foreach (json_decode($newz->image)as $picture) { ?>
                <img class="card-img-top" src="{{ asset('/imageupload/news/'.$picture) }}" alt="Card image cap">
              <?php } ?>
          </div> -->
          <div class="col-sm-8">
            <div class="card m-1 border-0">
              <div class="card-body pt-0">
                <?php foreach (json_decode($newz->image)as $picture) { ?>
                <img class="card-img-top border border-dark mb-3" src="{{ asset('/imageupload/news/'.$picture) }}" alt="Card image cap">
                <?php } ?>
                <h4 class="card-title">{{$newz->title}}</h4>
                <p class="card-text">{{$newz->description}}</p>
                <p class="card-text"><small class="text-muted">{{$newz->created_at}}</small></p>
              </div>
            </div>
          </div>
          <div class="col-sm-4">            
              <?php foreach (json_decode($newz->pdf)as $pdf) { ?>
                      <iframe src="{{ asset('/pdfupload/news/'.$pdf) }}" width="100%" height="500px">
              <?php } ?>
          </div>
          @endforeach
        </div>
